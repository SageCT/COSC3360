...
void *reverseIntArray(void *void_ptr) {
  arguments *arg_ptr = (arguments *)
      void_ptr; // cast the void pointer to a struct arguments pointer
  int pos1 = arg_ptr->pos;
  int pos2 = arg_ptr->size - 1 - arg_ptr->pos;
  swap(arg_ptr->array[pos1], arg_ptr->array[pos2]);
  // call the swap function to change the values between array[pos1] and
  // array[pos2]
...
int main() {
  ...
  for (int i = 0; i < nthreads; i++) {
    ...
    if (pthread_create(&tid[i], nullptr, reverseIntArray, (void *)&arg[i])) {
